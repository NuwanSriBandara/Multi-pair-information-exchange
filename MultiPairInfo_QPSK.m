...
h4 = sqrt(d4^-eta)*(randn(N/2,1) + 1i*randn(N/2,1))/sqrt(2);
h5 = sqrt(d5^-eta)*(randn(N/2,1) + 1i*randn(N/2,1))/sqrt(2);
h6 = sqrt(d6^-eta)*(randn(N/2,1) + 1i*randn(N/2,1))/sqrt(2);
...
n4 = sqrt(no)*(randn(N/2,1) + 1i*randn(N/2,1))/sqrt(2);
n5 = sqrt(no)*(randn(N/2,1) + 1i*randn(N/2,1))/sqrt(2);
n6 = sqrt(no)*(randn(N/2,1) + 1i*randn(N/2,1))/sqrt(2);
...
nc1 = sqrt(no)*(randn(N/2,1) + 1i*randn(N/2,1))/sqrt(2);
nc2 = sqrt(no)*(randn(N/2,1) + 1i*randn(N/2,1))/sqrt(2);
nc3 = sqrt(no)*(randn(N/2,1) + 1i*randn(N/2,1))/sqrt(2);
nc4 = sqrt(no)*(randn(N/2,1) + 1i*randn(N/2,1))/sqrt(2);
nc5 = sqrt(no)*(randn(N/2,1) + 1i*randn(N/2,1))/sqrt(2);
nc6 = sqrt(no)*(randn(N/2,1) + 1i*randn(N/2,1))/sqrt(2);
...
%Received signals
   y1_ = sqrt(pt(u))*C.*h1 + nc1;	%At user 1
   y2_ = sqrt(pt(u))*C.*h2 + nc2;	%At user 2
   y3_ = sqrt(pt(u))*C.*h3 + nc3;	%At user 3
...
%Decode at user 3
   ...
   received_3 = xor(dec3_,x6);
   
%BER calculation
   ber1_(u) = biterr(received_1, x1)/N;
   ber2_(u) = biterr(received_2, x2)/N;
   ber3_(u) = biterr(received_3, x3)/N;
end

for u = 1:length(Pt)
	
%Received signals
   y4_ = sqrt(pt(u))*C.*h4 + nc4;	%At user 1
   y5_ = sqrt(pt(u))*C.*h5 + nc5;	%At user 2
   y6_ = sqrt(pt(u))*C.*h6 + nc6;	%At user 3
...
%Decode at user 3
   ...
   received_6 = xor(dec6_,x3);
   
%BER calculation
   ber4_(u) = biterr(received_4, x4)/N;
   ber5_(u) = biterr(received_5, x5)/N;
   ber6_(u) = biterr(received_6, x6)/N;
end
...
semilogy(Pt, ber4_, '-o', 'linewidth', 2);
semilogy(Pt, ber5_, '-o', 'linewidth', 2);
semilogy(Pt, ber6_, '-o', 'linewidth', 2);
xlabel('Transmit power (dBm)');
ylabel('BER');
